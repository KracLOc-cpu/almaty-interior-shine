...
// Extra gallery items reusing existing images for the "More works" grid
const gallery = [
  { img: after1, key: "port_1" },
  { img: after2, key: "port_2" },
  { img: after3, key: "port_3" },
  { img: before1, key: "port_1" },
  { img: before2, key: "port_2" },
  { img: before3, key: "port_3" },
  { img: after1, key: "port_3" },
  { img: after2, key: "port_1" },
  { img: after3, key: "port_2" },
];
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 max-w-3xl mx-auto">
          {gallery.map((item, i) => (
            <div key={i} className="relative overflow-hidden rounded-lg aspect-square">
              <img src={item.img} alt={t(item.key)} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" />
            </div>
          ))}
        </div>
      </div>
